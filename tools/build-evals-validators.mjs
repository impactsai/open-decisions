...
ajv.addSchema(JSON.parse(readFileSync(new URL('../schemas/comparison/0.1.0-draft.1/contract.schema.json', import.meta.url))));
ajv.addSchema(schema);
const names = Object.keys(schema.$defs);
const code = '// Generated from the normative JSON Schema. Run npm run evals:generate.\n' +
  standalone(ajv, Object.fromEntries(names.map(n => [`v_${n}`, `${schema.$id}#/$defs/${n}`])));
...
const path = new URL('../evals/validators.mjs', import.meta.url);
if (process.argv.includes('--check')) {
  if (readFileSync(path, 'utf8') !== code) throw new Error('Decision Evals validators are stale');
